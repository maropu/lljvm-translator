...
class FunctionSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("functions for Numba") {
    ...
    val addr1 = org.maropu.lljvm.runtime.Function.invoke_i64("", "_NRT_MemInfo_alloc_safe_aligned(JI)J", args1)
    assert(addr1 != 0L)

    // Invokes `_NRT_MemInfo_call_dtor(addr1)`
    val args2 = VMemory.allocateStack(8)
    VMemory.pack(args2, addr1)
    org.maropu.lljvm.runtime.Function.invoke_void("", "_NRT_MemInfo_call_dtor(J)V", args2)
  ...
  test("non-existent functions") {
    val errMsg = intercept[LLJVMRuntimeException] {
      org.maropu.lljvm.runtime.Function.invoke_void("", "unknown function", 0)
    }.getMessage
